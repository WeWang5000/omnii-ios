//
//  PageLinkedList.swift
//  HYPageViewController
//
//  Created by huxiaoyang on 2023/4/11.
//
...
// MARK: - Extension to adapt PageObjectDiffable as node
extension LinkedList<PageDiffable> {
    
    /// Function to return the node for PageObjectDiffable.
    ///
    /// - Parameter identifier: PageObjectDiffable of the node's value to be returned
    /// - Returns: LinkedListNode
    public func node(for value: PageDiffable) -> Node? {
        assert(head != nil, "List is empty")
        
        var node = head
        while let nd = node {
            if nd.value.isEqual(to: value) {
                break
            }
            node = nd.next
        }
...
// MARK: - Extension to insert
extension LinkedList {
    
    public func insert(_ value: T, before node: Node) {
        let newNode = Node(value: value)
        insert(newNode, before: node)
    }
    
    public func insert(_ newNode: Node, before node: Node) {
    ...
    public func insert(_ value: T, after node: Node) {
        let newNode = Node(value: value)
        insert(newNode, after: node)
    }
    
    public func insert(_ newNode: Node, after node: Node) {
