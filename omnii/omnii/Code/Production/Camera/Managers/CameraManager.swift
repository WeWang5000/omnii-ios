...
open class CameraManager: NSObject, UIGestureRecognizerDelegate {
    ...
    fileprivate var photoCompletion: ((CaptureResult) -> Void)?
     ...
     */
    open func addPreviewLayerToView(_ view: UIView) {
        addPreviewLayerToView(view, newCameraOutputMode: cameraOutputMode)
    }
    
    open func addPreviewLayerToView(_ view: UIView, newCameraOutputMode: CameraOutputMode) {
        addLayerPreviewToView(view, newCameraOutputMode: newCameraOutputMode, completion: nil)
    }
    
    open func addLayerPreviewToView(_ view: UIView, newCameraOutputMode: CameraOutputMode, completion: (() -> Void)?) {
    ...
    open func capturePictureDataWithCompletion(_ imageCompletion: @escaping (CaptureResult) -> Void) {
        if self.animateShutter {
            self._performShutterAnimation {
                self._capturePictureDataWithCompletion(imageCompletion)
            }
        } else {
            _capturePictureDataWithCompletion(imageCompletion)
        }
    }
        ...
        if writeFilesToPhoneLibrary {
            sessionQueue.async {
                ...
                do {
                    ...
                    if PHPhotoLibrary.authorizationStatus() != .authorized {
                        PHPhotoLibrary.requestAuthorization { status in
                            if status == PHAuthorizationStatus.authorized {
                                self._saveImageToLibrary(atFileURL: filePath)
                            }
                        }
                    } else {
                        self._saveImageToLibrary(atFileURL: filePath)
                    ...
                    self._catchError("save error", message: error.localizedDescription)
                    return
                }
            }
        }
    ...
    fileprivate func _saveImageToLibrary(atFileURL filePath: URL) {
        ...
        library?.save(imageAtURL: filePath, albumName: imageAlbumName, date: date, location: location) { asset in
            ...
            guard let _ = asset else {
                self._catchError("save image error", message: "asset is not exist")
                return
            }
        }
    }
     ...
     */
    open func _capturePictureDataWithCompletion(_ imageCompletion: @escaping (CaptureResult) -> Void) {
        guard cameraIsSetup else {
            _catchError(NSLocalizedString("No capture session setup", comment: ""), message: NSLocalizedString("I can't take any picture", comment: ""))
            return
        }
        
        guard cameraOutputMode == .stillImage else {
            _catchError(NSLocalizedString("Capture session output mode video", comment: ""), message: NSLocalizedString("I can't take any picture", comment: ""))
            return
        }
        
        _updateIlluminationMode(flashMode)
        
        self.photoCompletion = imageCompletion
    ...
    open func startRecordingVideo() {
        guard cameraOutputMode != .stillImage else {
            _catchError(NSLocalizedString("Capture session output still image", comment: ""), message: NSLocalizedString("I can only take pictures", comment: ""))
            return
        }
    ...
    fileprivate func _zoom(_ scale: CGFloat) {
        ...
        do {
            ...
            self._catchError("Error locking configuration", message: error.localizedDescription)
        }
    }
    ...
    @objc fileprivate func _focusStart(_ recognizer: UITapGestureRecognizer) {
        ...
        if let validDevice = device,
            ...
            do {
                ...
                _catchErrorFocusRectangleAtPoint(pointInPreviewLayer, inLayer: validPreviewLayer)
                ...
                self._catchError("error lock", message: error.localizedDescription)
            }
        }
    }
    ...
    fileprivate func _catchErrorFocusRectangleAtPoint(_ focusPoint: CGPoint, inLayer layer: CALayer, withBrightness brightness: Float? = nil) {
    ...
    @objc fileprivate func _exposureStart(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        if let validPreviewLayer = previewLayer, let lastFocusPoint = self.lastFocusPoint {
            _catchErrorFocusRectangleAtPoint(lastFocusPoint, inLayer: validPreviewLayer, withBrightness: exposureValue)
        }
    }
    ...
    fileprivate func _videoOrientationFromStatusBarOrientation() -> AVCaptureVideoOrientation {
        ...
        DispatchQueue.main.async {
            orientation = UIApplication.shared.interfaceOrientation()
        }
    ...
    fileprivate func _checkIfCameraIsAvailable() -> CameraState {
        ...
        if deviceHasCamera {
            ...
            if userAgreedToUseIt {
                ...
                _catchError(NSLocalizedString("Camera access denied", comment: ""), message: NSLocalizedString("You need to go to settings app and grant acces to the camera device to use it.", comment: ""))
                return .accessDenied
            }
        } else {
            _catchError(NSLocalizedString("Camera unavailable", comment: ""), message: NSLocalizedString("The device does not have a camera.", comment: ""))
    ...
    open func _doFlipAnimation() {
        ...
        if let validEmbeddingView = embeddingView,
            ...
            if UIAccessibility.isReduceTransparencyEnabled {
                ...
                tempView = DynamicBlurView(frame: validEmbeddingView.bounds).then {
                    $0.blurRadius = 8.0
                    $0.trackingMode = .none
    ...
    fileprivate func _updateIlluminationMode(_ mode: AVCaptureDevice.FlashMode) {
        if cameraOutputMode != .stillImage {
            _updateTorch(mode)
        }
    }
    
    fileprivate func _updateTorch(_: AVCaptureDevice.FlashMode) {
        captureSession?.beginConfiguration()
    ...
    fileprivate func _updateCameraQualityMode(_ newCameraOutputQuality: AVCaptureSession.Preset) {
        if let validCaptureSession = captureSession {
            ...
            if validCaptureSession.canSetSessionPreset(sessionPreset) {
                ...
                _catchError(NSLocalizedString("Preset not supported", comment: ""), message: NSLocalizedString("Camera preset not supported. Please try another one.", comment: ""))
            }
        } else {
            _catchError(NSLocalizedString("Camera error", comment: ""), message: NSLocalizedString("No valid capture session found, I can't take any pictures or videos.", comment: ""))
        }
    }
    ...
    fileprivate func _catchError(_ title: String, message: String) {
        print("Camera \(title): \(message)")
    }
    ...
    fileprivate func _deviceInputFromDevice(_ device: AVCaptureDevice?) -> AVCaptureDeviceInput? {
        ...
        do {
            ...
            _catchError(NSLocalizedString("Device setup error occured", comment: ""), message: "\(outError)")
            return nil
        }
    }
...
extension CameraManager: AVCapturePhotoCaptureDelegate {
    ...
    fileprivate func capturePhoto() {
        let stillImageOutput = _getStillImageOutput()
        if let connection = stillImageOutput.connection(with: AVMediaType.video),
            ...
            if let completiom = photoCompletion {
                completiom(.failure(CaptureError.noVideoConnection))
            }
        }
    ...
    public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        if let error = error {
            self._catchError(NSLocalizedString("Error", comment: ""), message: error.localizedDescription)
            completion(.failure(CaptureError.noVideoConnection))
            return
        }
...
extension CameraManager: AVCaptureFileOutputRecordingDelegate {
    ...
    open func fileOutput(_: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from _: [AVCaptureConnection], error: Error?) {
        if let error = error {
            _catchError(NSLocalizedString("Unable to save video to the device", comment: ""), message: error.localizedDescription)
        } else {
            if writeFilesToPhoneLibrary {
                if PHPhotoLibrary.authorizationStatus() == .authorized {
